...
try:
    ...
    TK_OK = True
    logger.info("Tkinter and Matplotlib successfully initialized")
except Exception as e:
    TK_OK = False
    logger.warning(f"Tkinter/Matplotlib initialization failed: {e}")
...
def set_seed(s=SEED):
    ...
    logger.debug(f"Random seed set to {s}")
...
def show_results_popup(target_name,
    ...
    root = tk.Tk()
    root.title("FNN Inference Results — Metrics & Plots")
    ...
    metrics_text.write("==================== INFERENCE METRICS (Target: Power Output) ====================\n")
    ...
    ax1.set_title(f"Inference: True vs Predicted (R²={metrics['r2']:.3f}, MAE={metrics['mae']:.2f})")
...
def main():
    ...
    model_path = "best_fnn.keras"
    
    logger.info(f"Starting inference on: {csv_path}")
    logger.info(f"Expected model path: {model_path}")
    
    print(f"Reading test data: {csv_path}")
    df0 = pd.read_csv(csv_path)
    logger.info(f"Test data loaded: shape {df0.shape}")
    logger.debug(f"Columns: {list(df0.columns)}")

    if df0.shape[1] < 2:
        logger.error("Dataset has less than 2 columns")
        raise ValueError("Dataset must have at least 2 columns (datetime + target or id + target).")

    #Target is strictly the 2nd column
    target = df0.columns[1]
    print(f"Target column (2nd): {target}  [UI label: {NICE_TARGET_NAME}]")
    logger.info(f"Target column identified: {target}")

    #DateTime handling + resample hourly
    dt_col = find_datetime_col(df0)
    if dt_col is not None:
        logger.info(f"DateTime column found: {dt_col}")
        ...
        logger.info(f"DateTime processing complete: hourly resampling applied")
    else:
        logger.warning("No DateTime column found, proceeding without datetime handling")
        df = df0.copy()
        df[target] = pd.to_numeric(df[target], errors="coerce")
    ...
    if "DateTime" in df.columns:
        ...
        logger.debug("Time features added successfully")
    ...
    logger.debug(f"Lag and rolling features created")

    #Build numeric matrix & keep DateTime aligned
    logger.info("Building numeric feature matrix...")
    num_df = df.select_dtypes(include=[np.number]).copy()
    feature_cols = [c for c in num_df.columns if c != target]
    if len(feature_cols) == 0:
        logger.error("No numeric features found besides target")
    ...
    logger.info(f"After removing NaN values: {len(aligned)} samples")
    ...
    print(f"\nTest set shape: {X.shape}")
    logger.info(f"Test set shape: {X.shape} (samples={X.shape[0]}, features={X.shape[1]})")
    ...
    try:
        ...
        scalerX = scalers['scalerX']
        scalerY = scalers['scalerY']
        print("Loaded pre-fitted scalers from training.")
        logger.info("Scalers loaded successfully from scalers.pkl")
    except FileNotFoundError:
        logger.warning("Scalers not found. Creating new scalers (results may differ from training test set).")
        print("[WARN] Scalers not found. Creating new scalers (results may differ from training test set).")
        # Create new scalers if not available
        imputer = SimpleImputer(strategy="median").fit(X)
        scalerX = MinMaxScaler().fit(X)
        scalerY = RobustScaler().fit(y.reshape(-1,1))
        logger.warning("New scalers created from test data")

    # Preprocess test data
    logger.info("Preprocessing test data...")
    X_imputed = imputer.transform(X)
    logger.debug(f"Data imputed: shape {X_imputed.shape}")
    X_scaled = scalerX.transform(X_imputed)
    logger.debug(f"Features scaled: shape {X_scaled.shape}")

    # Make predictions
    print("Making predictions on test set...")
    logger.info("Making predictions on test set...")
    y_pred_scaled = model.predict(X_scaled, batch_size=BATCH, verbose=0).ravel()
    ...
    logger.debug(f"Basic metrics computed: R²={r2:.6f}, MAE={mae:.6f}, RMSE={rmse:.6f}")

    eps = 1e-12
    mean_abs_y = float(np.mean(np.abs(y)))
    mean_sq_y  = float(np.mean(y.astype("float64")**2))
    mae_pct  = 100.0 * mae  / max(mean_abs_y, eps)
    rmse_pct = 100.0 * rmse / max(mean_abs_y, eps)
    mse_pct  = 100.0 * mse  / max(mean_sq_y, eps)
    r2_pct   = 100.0 * r2
    logger.debug(f"Percentage metrics: MAE%={mae_pct:.6f}%, RMSE%={rmse_pct:.6f}%")

    print("\n==================== INFERENCE METRICS (Target: Power Output) ====================")
    print(f"R²    : {r2:8.4f}")
    ...
    logger.info("=" * 80)
    logger.info("INFERENCE METRICS SUMMARY")
    logger.info("=" * 80)
    logger.info(f"R²:    {r2:.6f}")
    logger.info(f"MSE:   {mse:.6f}")
    logger.info(f"RMSE:  {rmse:.6f}")
    logger.info(f"MAE:   {mae:.6f}")
    logger.info(f"MAE%:  {mae_pct:.6f}%")
    logger.info(f"RMSE%: {rmse_pct:.6f}%")
    logger.info(f"MSE%:  {mse_pct:.6f}%")
    logger.info(f"R²%:   {r2_pct:.6f}%")
    logger.info("=" * 80)

    # Save predictions (+ DateTime)
    logger.info("Saving predictions and metrics...")
    out = pd.DataFrame({"DateTime": dt_series, "power_true": y, "power_pred": y_pred})
    out.to_csv("inference_predictions_test.csv", index=False)
    print("\n[INFO] Predictions saved to inference_predictions_test.csv")
    logger.info("Predictions saved to inference_predictions_test.csv")

    with open("inference_metrics_test.txt","w") as f:
        f.write(
            f"R2={r2:.6f}\nMSE={mse:.6f}\nRMSE={rmse:.6f}\nMAE={mae:.6f}\n"
            f"MAE%={mae_pct:.6f}%\nRMSE%={rmse_pct:.6f}%\nMSE%={mse_pct:.6f}%\nR2%={r2_pct:.6f}%\n"
            f"mean_abs_power={mean_abs_y:.6f}\nmean_sq_power={mean_sq_y:.6f}\n"
        )
    logger.info("Metrics saved to inference_metrics_test.txt")
    print("[INFO] Metrics saved to inference_metrics_test.txt")

    #Build error table: remove zero-power, filter hours 07–15 and %err < 20
    logger.info("Building error analysis table...")
    abs_err = np.abs(y - y_pred)

    errors_df = pd.DataFrame({
        "DateTime": dt_series,
        "power_true": y.astype(float),
        "power_pred": y_pred.astype(float),
        "abs_err": abs_err.astype(float)
    })
    logger.debug(f"Error table created: {len(errors_df)} total rows")

    #Remove night-time / near-zero target
    errors_df = errors_df[errors_df["power_true"] > ZERO_TOL].copy()
    logger.debug(f"After removing zero-power: {len(errors_df)} rows")
    ...
    if errors_df["DateTime"].notna().any():
        ...
        logger.debug(f"After hour filtering ({HOUR_START:02d}:00–{HOUR_END:02d}:00): {len(errors_df)} rows")
    else:
        msg = f"[WARN] No valid DateTime available; cannot filter by hours {HOUR_START:02d}:00–{HOUR_END:02d}:00."
        print(msg)
        logger.warning(msg)
    ...
    logger.debug(f"After error threshold filtering (<{ERR_PCT_MAX}%): {len(errors_df)} rows")

    #Sort & save filtered table (no Index column)
    errors_df = errors_df.sort_values("pct_err", ascending=True).reset_index(drop=True)
    errors_df.to_csv("inference_errors_test.csv", index=False)
    print("[INFO] Error details saved to inference_errors_test.csv")
    logger.info(f"Error table saved to inference_errors_test.csv ({len(errors_df)} rows)")

    # Save prediction plot
    logger.info("Creating visualization plot...")
    try:
        import matplotlib.pyplot as plt
        plt.figure(figsize=(12,5))
        plt.plot(y, label="True Power Output", alpha=0.7)
        plt.plot(y_pred, label="Predicted Power Output", alpha=0.7)
        plt.title("Test Set: True vs Predicted Power Output")
        plt.xlabel("Sample Index")
        plt.ylabel("Power Output")
        plt.legend()
        plt.grid(True, alpha=0.3)
        plt.tight_layout()
        plt.savefig("inference_true_vs_pred_test.png", dpi=100)
        plt.close()
        print("[INFO] Plot saved to inference_true_vs_pred_test.png")
        logger.info("Visualization saved to inference_true_vs_pred_test.png")
    except Exception as e:
        print(f"[WARN] Could not save plot: {e}")
        logger.warning(f"Could not save plot: {e}")

    #POPUP WINDOW with filtered table
    logger.info("Opening results popup window...")
    metrics_pack = {
    ...
    show_results_popup(NICE_TARGET_NAME, y, y_pred, metrics_pack, errors_df)
